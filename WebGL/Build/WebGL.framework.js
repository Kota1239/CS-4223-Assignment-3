...
// Set to null to disable video input devices altogether.
...
function _JS_Accelerometer_Start(callback, frequency) {
  ...
  if (typeof Accelerometer === "undefined") {
    JS_DeviceMotion_add();
    // Must call before we set the callback
...
function jsAudioCreateChannel(callback, userData) {
  ...
  // We calculate rolloff ourselves.
  /**
...
function Microphone_CreateSoundClipForRecording(channels, length, sampleRate) {
  ...
  soundClip.recordStart = function(deviceId, loopRecording) {
    ...
    return Microphone_GetMediaStream(deviceId, soundClip.recordBuffer.numberOfChannels, soundClip.recordBuffer.sampleRate).then(function(stream) {
      ...
      // 100 ms chunks + 1 extra to account for header data
      ...
      mediaRecorder.onstop = function() {
        ...
        recordedBlob.arrayBuffer().then(function(compressedBuffer) {
          ...
          if (soundClip.recordBuffer === null) return;
      ...
      // Record in 100 ms chunks to allow looping of record buffer
      ...
      if (contextState === "running") {
        ...
        while (sound !== undefined) {
          ...
          if (sound.channel.source) {
            ...
            if (resumeChannel) sound.channel.resume(WEBAudio.audioContextResumeOffset);
...
function jsAudioGetMimeTypeFromType(fmodSoundType) {
  switch (fmodSoundType) {
   ...
   default:
    // Fallback to mp4 audio file for other types or if not set (works on most browsers)
    return "audio/mp4";
  }
}
...
function _JS_Video_SetLoop(video, loop) {
  ...
  if (loop) {
    ...
    // Poll loop completion at at 30fps
    v.lastSeenPlaybackTime = v.currentTime;
    v.onended = null;
  } else {
...
function wgpuReadAdapterOptions(options) {
  return options ? {
    ...
    // 'core' is omitted intentionally, since the spec says it is the default value.
...
/** @suppress{checkTypes} */ var _navigator_gpu_request_adapter_async = function(options, adapterCallback, userData) {
...
function wgpuWriteI53ToU64HeapIdx(heap32Idx, number) {
  ...
  HEAPU64[heap32Idx >>> 1 >>> 0] = BigInt(number);
}
...
function wgpuLinkParentAndChild(parent, childId, child) {
  ...
  (parent.derivedObjects ??= new Map).set(childId, child);
}
...
function wgpuReadI53FromU64HeapIdx(heap32Idx) {
  assert(heap32Idx != 0, "assert(heap32Idx != 0) failed!");
  assert(heap32Idx % 2 == 0, "assert(heap32Idx % 2 == 0) failed!");
  return Number(HEAPU64[heap32Idx >>> 1 >>> 0]);
}
...
function wgpuReadDeviceDescriptor(descriptor) {
  ...
  return {
    ...
    // TODO: Check code size here, redundant for release builds?
    "requiredFeatures": wgpuReadFeaturesBitfield(descriptor + 40)
  };
}

/** @suppress{checkTypes} */ var _wgpu_adapter_request_device_async = function(adapter, descriptor, deviceCallback, userData) {
...
function _wgpu_canvas_get_webgpu_context(canvasSelector) {
  ...
  // Return existing ID if user has called wgpu_canvas_get_webgpu_context() multiple times on the same canvas.
...
function _wgpu_command_encoder_begin_render_pass(commandEncoder, descriptor) {
  ...
  assert((descriptor >>> 0) % (4) == 0, "Unaligned pointer fault!");
  descriptor >>>= 2;
  let colorAttachments = [], numColorAttachments = HEAP32[descriptor + 4 >>> 0], colorAttachmentsIdx = wgpu_checked_shift(HEAPU32[descriptor + 2 >>> 0], 2), colorAttachmentsIdxDbl = colorAttachmentsIdx + 6 >>> 1, // Alias the view for HEAPF64.
  maxDrawCount = HEAPF64[descriptor >>> 1 >>> 0], depthStencilAttachment = HEAPU32[descriptor + 5 >>> 0];
  ...
  while (numColorAttachments--) {
    ...
    colorAttachments.push(v ? {
      ...
      // Awkward polymorphism: spec does not allow 'depthSlice' to be given a value (even 0) if attachment is not a 3D texture.
  ...
  let desc = {
    ...
    // 5 + 9==sizeof(WGpuRenderPassDepthStencilAttachment)
...
function wgpuReadRenderPipelineDescriptor(descriptor) {
  ...
  assert((descriptor >>> 0) % (4) == 0, "Unaligned pointer fault!");
  descriptor >>>= 2;
  let vertexBuffers = [], targets = [], vertexIdx = descriptor, numVertexBuffers = HEAP32[vertexIdx + 7 >>> 0], // +7 == WGpuVertexState.numBuffers
  vertexBuffersIdx = wgpu_checked_shift(HEAPU32[vertexIdx + 2 >>> 0], 2), // +2 == WGpuVertexState.buffers
  ...
  numTargets = HEAP32[fragmentIdx + 7 >>> 0], // +7 == WGpuFragmentState.numTargets
  targetsIdx = wgpu_checked_shift(HEAPU32[fragmentIdx + 2 >>> 0], 2), // +2 == WGpuFragmentState.targets
  depthStencilFormat = HEAPU32[depthStencilIdx >>> 0], multisampleCount = HEAPU32[multisampleIdx >>> 0], fragmentModule = HEAPU32[fragmentIdx + 6 >>> 0], pipelineLayoutId = HEAPU32[fragmentIdx + 10 >>> 0], // sizeof(WGpuPipelineLayout)
  desc;
...
function _wgpuDispatchWebGpuErrorEvent(device, callback, error, userData) {
  ...
  // error type)
  _wgpuReportErrorCodeAndMessage(device, callback, _wgpuErrorObjectToErrorType(error), error?.["message"], userData);
}
...
function _wgpu_encoder_end(encoder) {
  ...
  /* https://gpuweb.github.io/gpuweb/#render-pass-encoder-finalization:
        ...
        to release references manually. */ _wgpu_object_destroy(encoder);
}
...
function _wgpu_encoder_finish(encoder) {
  ...
  /* https://gpuweb.github.io/gpuweb/#command-encoder-finalization:
        ...
        release references manually. */ _wgpu_object_destroy(encoder);
...
var wasmImports = {
  /** @export */ GetJSLoadTimeInfo: _GetJSLoadTimeInfo,
  /** @export */ GetJSMemoryInfo: _GetJSMemoryInfo,
  /** @export */ JS_Accelerometer_IsRunning: _JS_Accelerometer_IsRunning,
  /** @export */ JS_Accelerometer_Start: _JS_Accelerometer_Start,
  /** @export */ JS_Accelerometer_Stop: _JS_Accelerometer_Stop,
  /** @export */ JS_CallAsLongAsNoExceptionsSeen: _JS_CallAsLongAsNoExceptionsSeen,
  /** @export */ JS_Cursor_SetImage: _JS_Cursor_SetImage,
  /** @export */ JS_Cursor_SetShow: _JS_Cursor_SetShow,
  /** @export */ JS_DOM_MapViewportCoordinateToElementLocalCoordinate: _JS_DOM_MapViewportCoordinateToElementLocalCoordinate,
  /** @export */ JS_DOM_UnityCanvasSelector: _JS_DOM_UnityCanvasSelector,
  /** @export */ JS_DownloadTextFile: _JS_DownloadTextFile,
  /** @export */ JS_Eval_OpenURL: _JS_Eval_OpenURL,
  /** @export */ JS_FileSystem_Initialize: _JS_FileSystem_Initialize,
  /** @export */ JS_FileSystem_Sync: _JS_FileSystem_Sync,
  /** @export */ JS_GetRandomBytes: _JS_GetRandomBytes,
  /** @export */ JS_Get_WASM_Size: _JS_Get_WASM_Size,
  /** @export */ JS_GravitySensor_IsRunning: _JS_GravitySensor_IsRunning,
  /** @export */ JS_GravitySensor_Start: _JS_GravitySensor_Start,
  /** @export */ JS_GravitySensor_Stop: _JS_GravitySensor_Stop,
  /** @export */ JS_Gyroscope_IsRunning: _JS_Gyroscope_IsRunning,
  /** @export */ JS_Gyroscope_Start: _JS_Gyroscope_Start,
  /** @export */ JS_Gyroscope_Stop: _JS_Gyroscope_Stop,
  /** @export */ JS_Init_ContextMenuHandler: _JS_Init_ContextMenuHandler,
  /** @export */ JS_Init_CopyPaste: _JS_Init_CopyPaste,
  /** @export */ JS_LinearAccelerationSensor_IsRunning: _JS_LinearAccelerationSensor_IsRunning,
  /** @export */ JS_LinearAccelerationSensor_Start: _JS_LinearAccelerationSensor_Start,
  /** @export */ JS_LinearAccelerationSensor_Stop: _JS_LinearAccelerationSensor_Stop,
  /** @export */ JS_Log_Dump: _JS_Log_Dump,
  /** @export */ JS_Log_StackTrace: _JS_Log_StackTrace,
  /** @export */ JS_MobileKeybard_GetIgnoreBlurEvent: _JS_MobileKeybard_GetIgnoreBlurEvent,
  /** @export */ JS_MobileKeyboard_GetKeyboardStatus: _JS_MobileKeyboard_GetKeyboardStatus,
  /** @export */ JS_MobileKeyboard_GetText: _JS_MobileKeyboard_GetText,
  /** @export */ JS_MobileKeyboard_GetTextSelection: _JS_MobileKeyboard_GetTextSelection,
  /** @export */ JS_MobileKeyboard_Hide: _JS_MobileKeyboard_Hide,
  /** @export */ JS_MobileKeyboard_SetCharacterLimit: _JS_MobileKeyboard_SetCharacterLimit,
  /** @export */ JS_MobileKeyboard_SetText: _JS_MobileKeyboard_SetText,
  /** @export */ JS_MobileKeyboard_SetTextSelection: _JS_MobileKeyboard_SetTextSelection,
  /** @export */ JS_MobileKeyboard_Show: _JS_MobileKeyboard_Show,
  /** @export */ JS_Module_WebGLContextAttributes_PowerPreference: _JS_Module_WebGLContextAttributes_PowerPreference,
  /** @export */ JS_Module_WebGLContextAttributes_PremultipliedAlpha: _JS_Module_WebGLContextAttributes_PremultipliedAlpha,
  /** @export */ JS_Module_WebGLContextAttributes_PreserveDrawingBuffer: _JS_Module_WebGLContextAttributes_PreserveDrawingBuffer,
  /** @export */ JS_OrientationSensor_IsRunning: _JS_OrientationSensor_IsRunning,
  /** @export */ JS_OrientationSensor_Start: _JS_OrientationSensor_Start,
  /** @export */ JS_OrientationSensor_Stop: _JS_OrientationSensor_Stop,
  /** @export */ JS_RequestDeviceSensorPermissionsOnTouch: _JS_RequestDeviceSensorPermissionsOnTouch,
  /** @export */ JS_RunQuitCallbacks: _JS_RunQuitCallbacks,
  /** @export */ JS_ScreenOrientation_DeInit: _JS_ScreenOrientation_DeInit,
  /** @export */ JS_ScreenOrientation_Init: _JS_ScreenOrientation_Init,
  /** @export */ JS_ScreenOrientation_Lock: _JS_ScreenOrientation_Lock,
  /** @export */ JS_SetMainLoop: _JS_SetMainLoop,
  /** @export */ JS_Sound_Create_Channel: _JS_Sound_Create_Channel,
  /** @export */ JS_Sound_Create_With_Buffer: _JS_Sound_Create_With_Buffer,
  /** @export */ JS_Sound_GetAudioBufferSampleRate: _JS_Sound_GetAudioBufferSampleRate,
  /** @export */ JS_Sound_GetAudioContextSampleRate: _JS_Sound_GetAudioContextSampleRate,
  /** @export */ JS_Sound_GetData: _JS_Sound_GetData,
  /** @export */ JS_Sound_GetLength: _JS_Sound_GetLength,
  /** @export */ JS_Sound_GetLoadState: _JS_Sound_GetLoadState,
  /** @export */ JS_Sound_GetMetaData: _JS_Sound_GetMetaData,
  /** @export */ JS_Sound_Init: _JS_Sound_Init,
  /** @export */ JS_Sound_Load: _JS_Sound_Load,
  /** @export */ JS_Sound_Load_PCM: _JS_Sound_Load_PCM,
  /** @export */ JS_Sound_Play: _JS_Sound_Play,
  /** @export */ JS_Sound_ReleaseInstance: _JS_Sound_ReleaseInstance,
  /** @export */ JS_Sound_ResumeIfNeeded: _JS_Sound_ResumeIfNeeded,
  /** @export */ JS_Sound_Set3D: _JS_Sound_Set3D,
  /** @export */ JS_Sound_SetListenerOrientation: _JS_Sound_SetListenerOrientation,
  /** @export */ JS_Sound_SetListenerPosition: _JS_Sound_SetListenerPosition,
  /** @export */ JS_Sound_SetLoop: _JS_Sound_SetLoop,
  /** @export */ JS_Sound_SetLoopPoints: _JS_Sound_SetLoopPoints,
  /** @export */ JS_Sound_SetPaused: _JS_Sound_SetPaused,
  /** @export */ JS_Sound_SetPitch: _JS_Sound_SetPitch,
  /** @export */ JS_Sound_SetPosition: _JS_Sound_SetPosition,
  /** @export */ JS_Sound_SetVolume: _JS_Sound_SetVolume,
  /** @export */ JS_Sound_Stop: _JS_Sound_Stop,
  /** @export */ JS_SystemInfo_GetAnimationFrameRate: _JS_SystemInfo_GetAnimationFrameRate,
  /** @export */ JS_SystemInfo_GetBrowserName: _JS_SystemInfo_GetBrowserName,
  /** @export */ JS_SystemInfo_GetBrowserVersionString: _JS_SystemInfo_GetBrowserVersionString,
  /** @export */ JS_SystemInfo_GetCanvasClientSize: _JS_SystemInfo_GetCanvasClientSize,
  /** @export */ JS_SystemInfo_GetDocumentURL: _JS_SystemInfo_GetDocumentURL,
  /** @export */ JS_SystemInfo_GetGPUInfo: _JS_SystemInfo_GetGPUInfo,
  /** @export */ JS_SystemInfo_GetMatchWebGLToCanvasSize: _JS_SystemInfo_GetMatchWebGLToCanvasSize,
  /** @export */ JS_SystemInfo_GetOS: _JS_SystemInfo_GetOS,
  /** @export */ JS_SystemInfo_GetPreferredDevicePixelRatio: _JS_SystemInfo_GetPreferredDevicePixelRatio,
  /** @export */ JS_SystemInfo_GetScreenSize: _JS_SystemInfo_GetScreenSize,
  /** @export */ JS_SystemInfo_GetStreamingAssetsURL: _JS_SystemInfo_GetStreamingAssetsURL,
  /** @export */ JS_SystemInfo_HasAstcHdr: _JS_SystemInfo_HasAstcHdr,
  /** @export */ JS_SystemInfo_HasCursorLock: _JS_SystemInfo_HasCursorLock,
  /** @export */ JS_SystemInfo_HasFullscreen: _JS_SystemInfo_HasFullscreen,
  /** @export */ JS_SystemInfo_HasWebGL: _JS_SystemInfo_HasWebGL,
  /** @export */ JS_SystemInfo_HasWebGPU: _JS_SystemInfo_HasWebGPU,
  /** @export */ JS_SystemInfo_IsMobile: _JS_SystemInfo_IsMobile,
  /** @export */ JS_UnityEngineShouldQuit: _JS_UnityEngineShouldQuit,
  /** @export */ JS_Video_CanPlayFormat: _JS_Video_CanPlayFormat,
  /** @export */ JS_Video_Create: _JS_Video_Create,
  /** @export */ JS_Video_Destroy: _JS_Video_Destroy,
  /** @export */ JS_Video_Duration: _JS_Video_Duration,
  /** @export */ JS_Video_EnableAudioTrack: _JS_Video_EnableAudioTrack,
  /** @export */ JS_Video_GetAudioLanguageCode: _JS_Video_GetAudioLanguageCode,
  /** @export */ JS_Video_GetNumAudioTracks: _JS_Video_GetNumAudioTracks,
  /** @export */ JS_Video_GetPlaybackRate: _JS_Video_GetPlaybackRate,
  /** @export */ JS_Video_Height: _JS_Video_Height,
  /** @export */ JS_Video_IsPlaying: _JS_Video_IsPlaying,
  /** @export */ JS_Video_IsReady: _JS_Video_IsReady,
  /** @export */ JS_Video_IsSeeking: _JS_Video_IsSeeking,
  /** @export */ JS_Video_LastUpdateTime: _JS_Video_LastUpdateTime,
  /** @export */ JS_Video_Pause: _JS_Video_Pause,
  /** @export */ JS_Video_Play: _JS_Video_Play,
  /** @export */ JS_Video_Seek: _JS_Video_Seek,
  /** @export */ JS_Video_SetEndedHandler: _JS_Video_SetEndedHandler,
  /** @export */ JS_Video_SetErrorHandler: _JS_Video_SetErrorHandler,
  /** @export */ JS_Video_SetLoop: _JS_Video_SetLoop,
  /** @export */ JS_Video_SetMute: _JS_Video_SetMute,
  /** @export */ JS_Video_SetPlaybackRate: _JS_Video_SetPlaybackRate,
  /** @export */ JS_Video_SetReadyHandler: _JS_Video_SetReadyHandler,
  /** @export */ JS_Video_SetSeekedHandler: _JS_Video_SetSeekedHandler,
  /** @export */ JS_Video_SetVolume: _JS_Video_SetVolume,
  /** @export */ JS_Video_Time: _JS_Video_Time,
  /** @export */ JS_Video_UpdateToTexture: _JS_Video_UpdateToTexture,
  /** @export */ JS_Video_Width: _JS_Video_Width,
  /** @export */ JS_WebCamVideo_GetNativeHeight: _JS_WebCamVideo_GetNativeHeight,
  /** @export */ JS_WebCamVideo_GetNativeWidth: _JS_WebCamVideo_GetNativeWidth,
  /** @export */ JS_WebCamVideo_GrabFrame: _JS_WebCamVideo_GrabFrame,
  /** @export */ JS_WebGPU_ImportVideoTexture: _JS_WebGPU_ImportVideoTexture,
  /** @export */ JS_WebGPU_SetCommandEncoder: _JS_WebGPU_SetCommandEncoder,
  /** @export */ JS_WebGPU_Setup: _JS_WebGPU_Setup,
  /** @export */ JS_WebPlayer_FinishInitialization: _JS_WebPlayer_FinishInitialization,
  ...
  /** @export */ navigator_gpu_get_preferred_canvas_format: _navigator_gpu_get_preferred_canvas_format,
  /** @export */ navigator_gpu_request_adapter_async: _navigator_gpu_request_adapter_async,
  /** @export */ wgpu_adapter_or_device_get_features: _wgpu_adapter_or_device_get_features,
  /** @export */ wgpu_adapter_or_device_get_limits: _wgpu_adapter_or_device_get_limits,
  /** @export */ wgpu_adapter_request_device_async: _wgpu_adapter_request_device_async,
  /** @export */ wgpu_buffer_get_mapped_range: _wgpu_buffer_get_mapped_range,
  /** @export */ wgpu_buffer_map_async: _wgpu_buffer_map_async,
  /** @export */ wgpu_buffer_read_mapped_range: _wgpu_buffer_read_mapped_range,
  /** @export */ wgpu_buffer_unmap: _wgpu_buffer_unmap,
  /** @export */ wgpu_canvas_context_configure: _wgpu_canvas_context_configure,
  /** @export */ wgpu_canvas_context_get_current_texture: _wgpu_canvas_context_get_current_texture,
  /** @export */ wgpu_canvas_get_webgpu_context: _wgpu_canvas_get_webgpu_context,
  /** @export */ wgpu_command_encoder_begin_compute_pass: _wgpu_command_encoder_begin_compute_pass,
  /** @export */ wgpu_command_encoder_begin_render_pass: _wgpu_command_encoder_begin_render_pass,
  /** @export */ wgpu_command_encoder_copy_buffer_to_buffer: _wgpu_command_encoder_copy_buffer_to_buffer,
  /** @export */ wgpu_command_encoder_copy_texture_to_buffer: _wgpu_command_encoder_copy_texture_to_buffer,
  /** @export */ wgpu_command_encoder_copy_texture_to_texture: _wgpu_command_encoder_copy_texture_to_texture,
  /** @export */ wgpu_compute_pass_encoder_dispatch_workgroups: _wgpu_compute_pass_encoder_dispatch_workgroups,
  /** @export */ wgpu_compute_pass_encoder_dispatch_workgroups_indirect: _wgpu_compute_pass_encoder_dispatch_workgroups_indirect,
  /** @export */ wgpu_device_create_bind_group: _wgpu_device_create_bind_group,
  /** @export */ wgpu_device_create_bind_group_layout: _wgpu_device_create_bind_group_layout,
  /** @export */ wgpu_device_create_buffer: _wgpu_device_create_buffer,
  /** @export */ wgpu_device_create_command_encoder: _wgpu_device_create_command_encoder,
  /** @export */ wgpu_device_create_command_encoder_simple: _wgpu_device_create_command_encoder_simple,
  /** @export */ wgpu_device_create_compute_pipeline: _wgpu_device_create_compute_pipeline,
  /** @export */ wgpu_device_create_pipeline_layout: _wgpu_device_create_pipeline_layout,
  /** @export */ wgpu_device_create_render_pipeline: _wgpu_device_create_render_pipeline,
  /** @export */ wgpu_device_create_sampler: _wgpu_device_create_sampler,
  /** @export */ wgpu_device_create_shader_module: _wgpu_device_create_shader_module,
  /** @export */ wgpu_device_create_texture: _wgpu_device_create_texture,
  /** @export */ wgpu_device_get_queue: _wgpu_device_get_queue,
  /** @export */ wgpu_device_pop_error_scope_async: _wgpu_device_pop_error_scope_async,
  /** @export */ wgpu_device_push_error_scope: _wgpu_device_push_error_scope,
  /** @export */ wgpu_device_set_uncapturederror_callback: _wgpu_device_set_uncapturederror_callback,
  /** @export */ wgpu_encoder_end: _wgpu_encoder_end,
  /** @export */ wgpu_encoder_finish: _wgpu_encoder_finish,
  /** @export */ wgpu_encoder_pop_debug_group: _wgpu_encoder_pop_debug_group,
  /** @export */ wgpu_encoder_push_debug_group: _wgpu_encoder_push_debug_group,
  /** @export */ wgpu_encoder_set_bind_group: _wgpu_encoder_set_bind_group,
  /** @export */ wgpu_encoder_set_pipeline: _wgpu_encoder_set_pipeline,
  /** @export */ wgpu_is_valid_object: _wgpu_is_valid_object,
  /** @export */ wgpu_object_destroy: _wgpu_object_destroy,
  /** @export */ wgpu_object_set_label: _wgpu_object_set_label,
  /** @export */ wgpu_pipeline_get_bind_group_layout: _wgpu_pipeline_get_bind_group_layout,
  /** @export */ wgpu_queue_submit_multiple_and_destroy: _wgpu_queue_submit_multiple_and_destroy,
  /** @export */ wgpu_queue_submit_one_and_destroy: _wgpu_queue_submit_one_and_destroy,
  /** @export */ wgpu_queue_write_buffer: _wgpu_queue_write_buffer,
  /** @export */ wgpu_queue_write_texture: _wgpu_queue_write_texture,
  /** @export */ wgpu_render_commands_mixin_draw: _wgpu_render_commands_mixin_draw,
  /** @export */ wgpu_render_commands_mixin_draw_indexed: _wgpu_render_commands_mixin_draw_indexed,
  /** @export */ wgpu_render_commands_mixin_draw_indexed_indirect: _wgpu_render_commands_mixin_draw_indexed_indirect,
  /** @export */ wgpu_render_commands_mixin_draw_indirect: _wgpu_render_commands_mixin_draw_indirect,
  /** @export */ wgpu_render_commands_mixin_set_index_buffer: _wgpu_render_commands_mixin_set_index_buffer,
  /** @export */ wgpu_render_commands_mixin_set_vertex_buffer: _wgpu_render_commands_mixin_set_vertex_buffer,
  /** @export */ wgpu_render_pass_encoder_set_scissor_rect: _wgpu_render_pass_encoder_set_scissor_rect,
  /** @export */ wgpu_render_pass_encoder_set_stencil_reference: _wgpu_render_pass_encoder_set_stencil_reference,
  /** @export */ wgpu_render_pass_encoder_set_viewport: _wgpu_render_pass_encoder_set_viewport,
  /** @export */ wgpu_texture_create_view: _wgpu_texture_create_view,
  /** @export */ wgpu_texture_create_view_simple: _wgpu_texture_create_view_simple
};
